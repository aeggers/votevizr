...
#' @return In addition to making the plot (if plot.it = T), the function returns a list consisting of a list of \code{positional.tie.line.endpoints} (one per pair of candidates) and the \code{intersection.point} of these tie lines. 
#' @examples
...
#' @rdname plot_positional_results
#' @export
plot.positional.result = function(v.vec, positional.s = .5, add.fp.result = T, fp.result.col = "black", fp.result.cex = 1, shading.cols = c("#E495A566", "#86B87566", "#7DB0DD66"), secondary.line.col = "gray", secondary.line.lwd = 2, vertex.labels = c("A", "B", "C"), main = NULL, plot.it = T, new = T, border = "black", space = .1, clipped = F, clipped.x.range = c(1/4, 3/4), clipped.y.range = c(0, 1/2), draw.positional.tie.lines = F){
  ...
  tpms = ternary_point_mats_from_p_vec_and_s_v2(c(pAB, pAC, pBA, pBC, pCA, pCB), positional.s)  
    ...
    if(plot.it & draw.positional.tie.lines){
    for(j in 1:3){
