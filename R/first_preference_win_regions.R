...
#'  \item \code{A}, \code{B}, \code{C}: add to one of the candidates' 
#'  first-preference win region
#'  } 
...
#' @export
positional_first_preference_win_regions <- function(result, s = .5, candidate_labels = c("A", "B", "C")){
  ...
  rownames(out) = NULL
  colnames(out) = c("candidate", candidate_labels)    
  out
...
#' @rdname first_preference_win_regions
#' @export
rcv_first_preference_win_regions <- function(result, candidate_labels = c("A", "B", "C")){
  ...
  out <- rbind(data.frame(candidate = "A", A_fp_mat),
        data.frame(candidate = "B", B_fp_mat),
        data.frame(candidate = "C", C_fp_mat)
        )
...
#' @rdname first_preference_win_regions
#' @export
condorcet_first_preference_win_regions <- function(result, candidate_labels = c("A", "B", "C"), if_cycle = "empty"){
  ...
  }else if((if_cycle == "A" & forward.cycle) | (if_cycle == "C" & !forward.cycle)){
    k_mat = rbind(AB.BC.int, AB.BC.int, AB.BC.int)
  }else if((if_cycle == "C" & forward.cycle) | (if_cycle == "B" & !forward.cycle)){
    k_mat = rbind(AB.AC.int, AB.AC.int, AB.AC.int)
  }else if((if_cycle == "B" & forward.cycle) | (if_cycle == "A" & !forward.cycle)){
  ...
  j = 1
  A_df <- data.frame("candidate" = "A", rbind(vertex_mat[j,], first_edge_mat[j,], first_inner_intersection_mat[j, ], k_mat[j,], second_inner_intersection_mat[j,], second_edge_mat[j,], vertex_mat[j,]))
  j = 2
  B_df <- data.frame("candidate" = "B", rbind(vertex_mat[j,], first_edge_mat[j,], first_inner_intersection_mat[j, ], k_mat[j,], second_inner_intersection_mat[j,], second_edge_mat[j,], vertex_mat[j,]))
  j = 3
  C_df <- data.frame("candidate" = "C", rbind(vertex_mat[j,], first_edge_mat[j,], first_inner_intersection_mat[j, ], k_mat[j,], second_inner_intersection_mat[j,], second_edge_mat[j,], vertex_mat[j,]))
