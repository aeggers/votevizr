...
#'  yields \code{c(v_ab, v_ac, v_ba, v_bc, v_ca, v_cb, v_ax, v_bx, v_cx)}, where e.g. \code{v_ab} is the share of ballots listing \code{a} first and \code{b} second (implicitly \code{c} third), and \code{v_ax} indicates the share of ballots listing candidate \code{a} only. 
#' @export
extract_vector_of_vote_shares_and_candidate_names_from_result <- function(result, split = ""){
  ...
  if(length(candidate_names) != 3){
    stop(paste0("I expected 3 candidate names but I detected ", length(candidate_names), ": ", candidate_names, ".\nMore than three candidates, or incorrect 'split' argument?"))
  }
