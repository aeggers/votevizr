...
df <- tibble(
  `Preference ordering` = c("Remain > Deal > No deal",
               "No deal > Deal > Remain",
               "Deal > Remain > No deal",
               "Remain > No deal > Deal",
               "Deal > No deal > Remain",
...
Our starting point is the *ternary diagram* or *barycentric plot*, which has been used to represent plurality election outcomes for a long time. 
...
Because these numbers sum to 1, we could plot them in Remain-Deal space, as at left below.
...
plot(c(0,1) + .2*c(-1,1), c(0,1) + .2*c(-1,1), type = "n", xlab = "", ylab = "", axes = F, main = "Standard coordinates")
...
add.ternary.gridlines()
```

The basic idea of the ternary diagram is to represent a set of shares (points on the simplex) in a way that maintains symmetry among the alternatives.
...
par(mar = c(1,1,1,1))
green_shades <- c(rgb(229/255, 245/255, 224/255, alpha = .6), 
         rgb(161/255, 217/255, 155/255, alpha = .6),
         rgb(49/255, 163/255, 84/255, alpha = .6))
plot_plurality_result(brexit_poll, vertex.labels = c("Remain", "Deal", "No deal"), shading.cols = green_shades)
add.ternary.gridlines()
```
...
plot_plurality_result(brexit_poll, vertex.labels = c("Remain", "Deal", "No deal"), shading.cols = green_shades, main = "Plurality")
add.ternary.gridlines()
plot_borda_result(brexit_poll, vertex.labels = c("Remain", "Deal", "No deal"), shading.cols = green_shades, main = "Borda count")
add.ternary.gridlines()
```
...
par(mar = c(1,1,1,1), mfrow = c(1,2))
plot_antiplurality_result(brexit_poll, vertex.labels = c("Remain", "Deal", "No deal"), shading.cols = green_shades, main = "Anti-plurality (middle ranking worth 1)")
add.ternary.gridlines()
plot_positional_result(brexit_poll, positional.s = 2/5, vertex.labels = c("Remain", "Deal", "No deal"), shading.cols = green_shades, main = "Positional method with middle ranking worth 2/5")
add.ternary.gridlines()
```
...
par(mar = c(1,1,1,1))
plot_condorcet_result(brexit_poll, vertex.labels = c("Remain", "Deal", "No deal"), shading.cols = green_shades, main = "Condorcet methods (cycle in white triangle)")
add.ternary.gridlines()
```
...
par(mar = c(1,1,1,1), mfrow = c(1,2))
plot_condorcet_result(brexit_poll, vertex.labels = c("Remain", "Deal", "No deal"), shading.cols = green_shades, main = "Kemeny-Young method", in.cycle = "kemeny")
add.ternary.gridlines()
plot_condorcet_result(brexit_poll, vertex.labels = c("Remain", "Deal", "No deal"), shading.cols = green_shades, main = "Kemeny-Young method (zoomed in)", in.cycle = "kemeny", xlim = c(2/5, 3/5), ylim = sqrt(3/4)*c(1/10, 3/10), draw.majority.tie.lines = T, secondary.line.col = "black")
```
...
The best way to understand exactly what these figures is to read my paper, forthcoming in Social Choice and Welfare. 
